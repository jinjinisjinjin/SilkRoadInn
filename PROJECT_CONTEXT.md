...
> AI 代码助手请先读根目录 `MODEL_HANDOFF.md`，再读本文件，并继续阅读 `docs/DEVELOPMENT_MEMORY.md`。三者分别记录当前交接、长期结构，以及从开发问题中沉淀出的可复用工作经验。
...
│   ├── board-baseline-v1/      # 早期棋盘基线，已纳入 Git，保留作对照
│   └── scroll-v0.1/            # 长卷修缮原型
│       ├── index.html
│       ├── app.js
│       └── styles.css
...
│   ├── story/                  # 剧本、NPC 台词
│   ├── data-spec/              # 数据配置与资产管线
│   └── previews/               # 本地大体积预览输出，已 ignore，不纳入 Git
├── output/                     # 本地验收截图与临时输出，已 ignore，不纳入 Git
├── tools/                      # 开发工具
...
- **剧情与修缮**: 四章、22 个修缮节点、章节剧情、通关奖励和剧情回顾已经接通；没有可回顾内容时显示“尚未有纪事”空状态
...
3. **game/playable-v0.1/assets/ 是运行时资源**，当前页面的 HTML、CSS、JS 直接引用这里；根目录 `assets/` 是母版和整理稿，新图片不能只放在根目录 `assets/`。
...
不要使用 `tools/preview/start-preview.sh` 作为当前入口。该脚本仍指向旧的 `outputs/丝路食肆_可玩原型_v0.1`，与当前正式目录不一致。
